...
def _entries():
    ...
    return {
        ...
        "kdv_step": lambda: to_sympy(kdv_step, np.cosh(np.linspace(-3, 3, 32)) ** -2, 1e-4, 0.1),
        "kdv_step_bad (bug visible in formula)": lambda: to_sympy(kdv_step_bad, np.cosh(np.linspace(-3, 3, 32)) ** -2, 1e-4, 0.1),
        "markov_birth_death": lambda: to_sympy(birth_death_step, np.full(16, 1 / 16), 0.3, 0.2),
        ...
        "scipy trapezoid 2-D axis=1": lambda: to_sympy(lambda y: trapezoid(y, dx=0.1, axis=1), Y2),
        ...
        "np.average weighted": lambda: to_sympy(np.average, np.arange(5.0), None, np.arange(1.0, 6.0)),
        ...
        "np.gradient [tracker: guards/coercion]": lambda: to_sympy(np.gradient, 4 * np.linspace(0, 20, 8)),
        "np.polyval [tracker: guards]": lambda: to_sympy(np.polyval, np.array([2.0, 3.0, 5.0]), np.arange(4.0)),
        "np.median [tracker: guards]": lambda: to_sympy(np.median, np.arange(5.0)),
        "np.clip [tracker: table entry]": lambda: to_sympy(np.clip, np.linspace(-1, 1, 6), 0.0, 0.5),
    }
